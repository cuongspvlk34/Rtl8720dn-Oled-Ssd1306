...
void drawButton(int x, int y, int width, const char* label, bool highlight) {
  ...
  // Draw the button
  if (highlight) {
    display.fillRoundRect(x, y, width, height, radius, SSD1306_WHITE);  // Filled for highlight
    display.setTextColor(SSD1306_BLACK);                                // Inverted text color
  } else {
    display.drawRoundRect(x, y, width, height, radius, SSD1306_WHITE);  // Outline for non-highlight
    display.setTextColor(SSD1306_WHITE);
  }
...
void drawInputBox(int x, int y, const char* label, bool highlight) {
  ...
  // Draw the box
  if (highlight) {
    display.fillRect(x, y, width, height, SSD1306_WHITE);  // Filled box for highlight
    display.setTextColor(SSD1306_BLACK);                   // Inverted text color
    display.setCursor(128 - 10, y + 5);
    display.print("<");
  } else {
    display.drawRect(x, y, width, height, SSD1306_WHITE);  // Outline box for non-highlight
    display.setTextColor(SSD1306_WHITE);
  }
...
void _draw90InCorners() {
  ...
  display.drawRect(0, 0, width1, height1, SSD1306_WHITE);  // Horizontal
  display.drawRect(0, 0, width2, height2, SSD1306_WHITE);  // Vertical

  // Top-right corner (reverted angle)
  display.drawRect(128 - width1, 0, width1, height1, SSD1306_WHITE);  // Horizontal
  display.drawRect(128 - width2, 0, width2, height2, SSD1306_WHITE);  // Vertical

  // Bottom-left corner (reverted angle)
  display.drawRect(0, 64 - height1, width1, height1, SSD1306_WHITE);  // Horizontal
  display.drawRect(0, 64 - height2, width2, height2, SSD1306_WHITE);  // Vertical

  // Bottom-right corner (reverted angle)
  display.drawRect(128 - width1, 64 - height1, width1, height1, SSD1306_WHITE);  // Horizontal
  display.drawRect(128 - width2, 64 - height2, width2, height2, SSD1306_WHITE);  // Vertical
  display.display();
}
