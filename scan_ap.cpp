...
void printLegend(bool isShowing, uint8_t numerator, uint8_t denominator) {
  ...
  display.setTextColor(SSD1306_WHITE);  // Set text color to white (or your preferred color)
  ...
  uint8_t y = 64 - textHeight;  // Bottom of the screen, adjusted for text height
...
void drawRSSIStrength(int x, uint8_t y, int rssi) {
  ...
  for (int i = 0; i < bars; i++) {
    ...
    display.fillRect(barX, barY, lineWidth, barHeight, SSD1306_WHITE);
  }
}
