...
void _showMenuItem(uint8_t index, Direction direction) {
  ...
  while (step <= animationSteps) {
    ...
    if (currentTime - lastFrameTime >= frameTime) {
      ...
      display.fillRect(0, 20, 128, 100, SSD1306_BLACK);

      // Draw the bitmap at the current position
      display.drawBitmap(currentX, 40, menuItemIcons[index], 48, 48, SSD1306_WHITE);
...
void _showAuthorInfo() {
  display.drawBitmap(10, 3, bitmap_header, 109, 20, SSD1306_WHITE);
  display.drawBitmap(30, (128 - 10), footer_logo, 75, 11, SSD1306_WHITE);
  display.display();
}
